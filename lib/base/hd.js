...
module.exports = class HD {
    constructor(coin = "BTC") {
        ...
        this.coinPath = `m/44'/${coinTypeCode}'/1'/0/`
    }


    async getHDPrivateKey(index) {
        let _mnemonic = _coinType.mnemonic || mnemonic
        const seed = await bip39.mnemonicToSeed(_mnemonic)
        return this._getPrivateKey(seed, index)
    }

    async _getPrivateKey(seedBuff, index) {
        const root = bip32.fromSeed(seedBuff) // 可加入网络信息 得到wif 格式的私钥
        const path = this.coinPath + index
