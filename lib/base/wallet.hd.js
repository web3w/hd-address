...
let mnemonicStr = key.mnemonic
const CoinType = key.coin_type

let getCoinPath = (coin, index) => {
    let _coinType = CoinType[coin]
    let coinTypeCode = _coinType ? _coinType.code : 1 //coinType["TEST"].code
    return `m/44'/${coinTypeCode}'/1'/0/${index}`
}

let getPrivateKey = (seedBuff, path) => {
...
module.exports = class HD {
    constructor(mnemonic) {
        if (mnemonic) {
            ...
            console.warn("----------- mnemonic undefind, pleass user myself mnemonic ------------")
        }
    }

    async getHDPrivateKey(coin, index) {
        const seed = await bip39.mnemonicToSeed(mnemonicStr)
        const path = getCoinPath(coin, index)
        return getPrivateKey(seed, path)
    }

    // m/44'/0'/1'
    async getAccountChainCode(accountPath) {
        const seed = await bip39.mnemonicToSeed(mnemonicStr)
        const root = bip32.fromSeed(seed) // 可加入网络信息 得到wif 格式的私钥
        const child = root.derivePath(accountPath)
        let pri = child.privateKey //buffer
        let pub = child.publicKey //buffer
